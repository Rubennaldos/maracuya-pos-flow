...
import { getNextWeekDays, getEnabledDays, isDatePast } from "@/components/modules/lunch/utils/dateUtils";
...
export default function PedidosModule({
  ...
  const addLunchToCart = (product: ProductT) => {
    // ⛔️ Importante: ya NO salimos si falta specificDate.
    // Así combos/promos sin fecha se agregan normal.
    setCart((prev) => {
                ...
                >
                  {categories.map((cat) => (
                    <Button
                      ...
                      className={`rounded-full px-4 h-10 text-xs sm:text-sm font-semibold whitespace-nowrap flex-shrink-0 transition-all duration-300 ease-out ${
                        activeCat === cat.id
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`h-1 rounded-full transition-all duration-300 ${
                        activeCat === cat.id ? "w-6 bg-primary shadow-sm" : "w-1 bg-muted-foreground/30 hover:bg-muted-foreground/50"
                      }`}
            ...
            <Card className="sticky top-20">
              ...
              <CardContent className="space-y-3">
                {Object.keys(cart).length === 0 ? (
                  <p className="text-muted-foreground text-center py-8 text-sm">Carrito vacío</p>
                ) : (
                  <>
                      ...
                      {Object.entries(cart).map(([key, item]) => (
                        <div key={key} className="flex gap-2 items-start p-2 rounded-lg bg-muted/30">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => removeFromCart(key)} className="h-7 w-7 p-0 rounded-full">
                              -
                            </Button>
                          </div>
                        </div>
        ...
        <div className="lg:hidden fixed bottom-0 inset-x-0 bg-background/95 backdrop-blur border-t shadow-lg z-50">
          <div className="px-3 py-3">
            <div className="flex items-center justify-between gap-3 mb-2">
              ...
              <Button size="sm" className="rounded-full px-6 h-10 shadow-md" onClick={openConfirm} disabled={posting}>
                {posting ? "Enviando..." : "Confirmar"}
              </Button>
            </div>
            <Button variant="ghost" size="sm" onClick={clearCart} className="w-full h-8 text-xs">
              Limpiar carrito
            </Button>
          </div>
        </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input id="studentName" value={confirmStudent} onChange={(e) => setConfirmStudent(e.target.value)} />
            </div>
